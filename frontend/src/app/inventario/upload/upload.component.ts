...
export class UploadComponent {
  ...
  onUpload(): void {
    ...
    this.inventarioService.subirArchivoInventario(this.file)
      .subscribe({
        next: (res) => {
          this.showMessage('Archivo cargado con éxito: ' + res, 'success');
          this.loading = false;
        },
        error: (err) => {
          console.error('Error al subir archivo:', err);
          this.showMessage('Error al subir archivo: ' + (err.error || err.message || 'Error desconocido'), 'error');
          this.loading = false;
        }
      });
  }
  ...
  onTestUpload(): void {
    ...
    this.inventarioService.testUpload(this.file)
      .subscribe({
        next: (res) => {
          this.showMessage('Prueba exitosa: ' + res, 'success');
          this.loading = false;
        },
        error: (err) => {
          console.error('Error en prueba:', err);
          this.showMessage('Error en prueba: ' + (err.error || err.message || 'Error desconocido'), 'error');
          this.loading = false;
        }
      });
  }
